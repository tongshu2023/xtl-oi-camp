...
${body.split('\n').map(line => `    ${line}`).join('\n')}
  ...
  const profiles = [
    ...
    {
      match: /交换两个变量|算术|表达式|优先级|整除|取余|单位换算|坐标|距离|交换/,
...
for(auto x:a) cout<<x.id<<" "<<x.score<<"\\n"; }`,
    ...
    {
      match: /选择排序|冒泡排序|插入排序|快速排序|基础排序|归并|快排|排序/,
...
int main(){int n,m;cin>>n>>m;p.resize(n+1);iota(p.begin(),p.end(),0);while(m--){int a,b;cin>>a>>b;a=find(a);b=find(b);if(a!=b)p[a]=b;}int x,y;cin>>x>>y;cout<<(find(x)==find(y)?"YES":"NO")<<"\\n";}`,
    ...
    {
      match: /递推关系/,
      core: '递推用规模更小、已经确定的状态推出当前状态，必须同时给出初值、递推式和计算方向。',
      rule: '先定义第 i 项表示什么，再找它与前若干项的关系；从已知初值按依赖方向逐项计算。',
      example: ['铺台阶方案数', '每次走 1 或 2 阶，求走到第 n 阶的方案数。', '最后一步来自 n-1 或 n-2，因此 f[i]=f[i-1]+f[i-2]，f[0]=1。'],
      code: cpp('int n; cin >> n;\nvector<long long> f(max(2, n + 1));\nf[0] = 1; f[1] = 1;\nfor (int i = 2; i <= n; i++) f[i] = f[i-1] + f[i-2];\ncout << f[n] << \'\\n\';'),
      practice: ['兔子每月繁殖模型怎样写递推式？', '为什么只写 f[i]=f[i-1]+f[i-2] 还不完整？'],
      answer: ['先明确题设初始月份，再按“本月已有+本月新生”建立关系。', '没有状态含义、初值与计算范围，递推无法落地。'],
      trap: '递推式正确但初值错一位，导致后面全部偏移。',
      check: ['一个可执行的递推定义必须包含？', ['只有公式', '状态含义、初值、递推式和顺序', '只有样例', '只需循环'], 1, '四部分共同决定递推结果。']
    },
    {
      match: /线性动态规划|一维动态规划/,
      core: '线性动态规划按下标从左到右处理序列，当前最优通常只依赖前面少量状态。',
      rule: '定义 dp[i] 为“处理到第 i 个位置”的答案，枚举最后一步来自哪里，并保证所依赖位置已经算出。',
      example: ['最大不相邻和', '从一列非负数中选若干个，不能选相邻元素，求最大和。', 'dp[i]=max(dp[i-1],dp[i-2]+a[i])，分别表示不选或选择第 i 个。'],
      code: cpp('int n; cin >> n;\nvector<long long> a(n + 1), dp(n + 1);\nfor (int i = 1; i <= n; i++) cin >> a[i];\nfor (int i = 1; i <= n; i++) dp[i] = max(dp[i-1], (i >= 2 ? dp[i-2] : 0) + a[i]);\ncout << dp[n] << \'\\n\';'),
      practice: ['写出爬楼梯方案数的状态与转移。', '若只依赖前两项，空间怎样降到 O(1)？'],
      answer: ['dp[i] 表示到第 i 阶的方案数，dp[i]=dp[i-1]+dp[i-2]。', '只保存 prev2、prev1 和当前值并滚动更新。'],
      trap: 'dp[i] 的含义在转移过程中改变，或 i=1 时访问 dp[-1]。',
      check: ['线性 DP 最先要写清什么？', ['变量名长度', 'dp[i] 的确切含义', '使用哪种 IDE', '输出颜色'], 1, '状态定义决定转移和答案位置。']
    },
    {
      match: /二维动态规划|图与动态规划综合/,
      core: '二维动态规划用两个维度描述位置或阶段，例如网格坐标；每个格子的值来自已处理的相邻状态。',
      rule: '画出状态表，标清起点、不可达值和遍历方向；只有上方和左方已算出时，才能从它们转移到当前格。',
      example: ['网格最小路径和', '从左上角走到右下角，只能向右或向下，求经过数字之和最小值。', 'dp[i][j]=a[i][j]+min(dp[i-1][j],dp[i][j-1])，边界外设为无穷。'],
      code: cpp('int n,m; cin >> n >> m;\nconst long long INF = (1LL<<60);\nvector<vector<long long>> dp(n+1, vector<long long>(m+1, INF));\ndp[0][1] = 0;\nfor(int i=1;i<=n;i++) for(int j=1;j<=m;j++){long long x;cin>>x;dp[i][j]=x+min(dp[i-1][j],dp[i][j-1]);}\ncout << dp[n][m] << \'\\n\';'),
      practice: ['若网格有障碍，状态怎样表示不可达？', '为什么不能从右方或下方转移？'],
      answer: ['把障碍格保持为 INF 或方案数 0，不参与有效转移。', '按当前遍历顺序它们尚未计算，也不符合只能向右或向下的前驱关系。'],
      trap: '第一行、第一列边界初始化错误，或让不可达状态参与最小值。',
      check: ['网格 DP 的遍历顺序由什么决定？', ['屏幕大小', '状态依赖方向', '变量字母', '输入行数奇偶'], 1, '所有前驱状态必须先于当前状态计算。']
    },
    {
      match: /区间动态规划/,
      core: '区间动态规划把连续区间 [l,r] 作为状态，答案通常由枚举最后一次分割点合并两个更短区间得到。',
      rule: '先枚举区间长度，再枚举左端点和分割点；这样转移所需的短区间已经计算。',
      example: ['石子合并', '相邻两堆可合并，代价是两堆总重量，求全部合并的最小代价。', 'dp[l][r]=min(dp[l][k]+dp[k+1][r]+sum(l,r))。'],
      code: cpp('int n;cin>>n;vector<long long>s(n+1);for(int i=1,x;i<=n;i++){cin>>x;s[i]=s[i-1]+x;}vector<vector<long long>>dp(n+1,vector<long long>(n+1));\nfor(int len=2;len<=n;len++)for(int l=1;l+len-1<=n;l++){int r=l+len-1;dp[l][r]=1LL<<60;for(int k=l;k<r;k++)dp[l][r]=min(dp[l][r],dp[l][k]+dp[k+1][r]+s[r]-s[l-1]);}\ncout<<dp[1][n]<<\'\\n\';'),
      practice: ['为什么必须先枚举区间长度？', '转移中前缀和解决什么问题？'],
      answer: ['长区间依赖更短区间，按长度递增能保证依赖已算。', 'O(1) 得到任意区间重量，避免转移时重复求和。'],
      trap: '分割点枚举到 r，产生空区间或自我依赖。',
      check: ['区间 DP 最常见的第一层循环是？', ['数值大小', '区间长度', '答案大小', '随机顺序'], 1, '短区间必须先于长区间完成。']
    },
    {
      match: /动态规划最值优化/,
      ...
      check: ['DP 优化的安全前提是？', ['代码更短', '状态语义与依赖关系保持正确', '变量更少', '样例能过'], 1, '优化不能改变原问题的状态含义和转移依据。']
    },
    {
...
using namespace std;int main(){int n,m;cin>>n>>m;vector<vector<pair<int,int>>>g(n+1);while(m--){int u,v,w;cin>>u>>v>>w;g[u].push_back({v,w});}const long long INF=4e18;vector<long long>d(n+1,INF);priority_queue<pair<long long,int>,vector<pair<long long,int>>,greater<pair<long long,int>>>q;d[1]=0;q.push({0,1});while(!q.empty()){auto[du,u]=q.top();q.pop();if(du!=d[u])continue;for(auto[v,w]:g[u])if(d[v]>du+w){d[v]=du+w;q.push({d[v],v});}}for(int i=1;i<=n;i++)cout<<d[i]<<" ";}`,
    ...
    {
      match: /三角、对数与指数函数|常用数学函数|数学函数|平方根|绝对值|三角|对数|指数/,
    ...
    {
      match: /杨辉三角|计数原理|排列与组合|排列|组合|杨辉/,
...
using namespace std;int main(){ifstream fin("score.txt");if(!fin){cerr<<"open failed\\n";return 1;}string name;int score,sum=0,n=0;while(fin>>name>>score){sum+=score;n++;}if(n)cout<<(double)sum/n<<"\\n";}`,
  ...
  function pickProfile(title) {
    const matches = profiles
      .map((profile, index) => ({ profile, index, text: title.match(profile.match)?.[0] || '' }))
