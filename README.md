...
本站把无障碍（a11y）当成教学基础设施，而不是事后补丁——信奥课堂里既有用读屏软件的视障学习者，也有靠翻页笔连续讲课的老师，两类人都被认真照顾到：

- **跳过重复导航**：每页开头注入默认隐藏、聚焦才现身的 `.skip-link`「跳到主内容」，键盘/读屏用户按一次 Tab 即可越过导航直达正文（WCAG 2.4.1 Bypass Blocks）。
- **唯一主地标**：每页恰有一个 `<main id="main-content" tabindex="-1">`，读屏可一键跳转、脚本可编程聚焦，契约测试锁死「全页只有一个可聚焦主地标」。
- **投屏翻页不丢焦点**：教师投屏视图每翻一页都会整体重建 DOM；已做焦点恢复，上一页/下一页/揭晓答案后焦点自动落回对应按钮，用翻页笔或键盘可连续讲完整节课，不必每页重新找焦点。
- **读屏播报进度**：投屏区挂了一个常驻 `<body>`、不随重建销毁的 `aria-live="polite"` 活区 `#sr-announcer`，翻页时播报「第 N / 共 M 页」与标题，视障师生也能跟上讲到哪一张。
- **装饰元素消音**：投屏页码圆点、纯装饰徽标等标 `aria-hidden="true"`，不污染读屏朗读队列；弹窗为 `role="dialog" aria-modal`，表单错误用 `aria-live="polite"` 即时播报。

这些行为都有**零第三方依赖**的源码契约回归护栏（`node:test`）锁定，改动前后 `npm test` 即可挡住无障碍回归。
