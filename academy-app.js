(function () {
  ...
  const allCards = () => lessons.flatMap(x => x.memory.map(card => Object.assign({ lessonId: x.id, lessonTitle: x.title }, card)));
  ...
  function homePage() {
    const s = store.get(); const done = s.completedLessons.length; const due = store.dueCards(allCards()).length;
    return shell(`
      <section class="hero"><div class="hero-copy"><div class="eyebrow">2026 CSP-J 备考主站</div><h1>学会、记住、做对，<br><em>一直带到赛场。</em></h1><p>老师直播投屏、学生课后闯关、真题限时模考和记忆复习都在同一个网站里。</p><div class="hero-actions"><button class="primary" data-route="camp">进入 CSP-J 集训课</button><button class="secondary" data-route="plan">生成冲刺计划</button></div></div>
        <div class="hero-panel"><div class="countdown"><span>距第一轮</span><strong>${daysToExam()}</strong><b>天</b></div><div class="today-card"><span>今天建议</span><strong>${due ? `先复习 ${Math.min(due, 12)} 张到期记忆卡` : done < lessons.length ? `继续攻克 ${lessons[done].id} · ${lessons[done].title}` : '做一套完整真题卷'}</strong><button data-route="${due?'memory':'camp'}">现在开始 →</button></div><div class="mini-stats"><div><strong>${done}/${lessons.length}</strong><span>初赛关卡</span></div><div><strong>${due}</strong><span>到期记忆</span></div><div><strong>${s.quizAttempts.length}</strong><span>小测次数</span></div></div></div>
      </section>
      <section class="section"><div class="section-head"><div><span class="kicker">三条学习路径</span><h2>所有信奥内容，回到同一个入口</h2></div><p>不让学生在链接、课件、题库之间来回找。</p></div>
        <div class="path-grid">
          <button class="path-card regular" data-route="regular"><span class="path-index">01</span><div class="path-icon">{ }</div><h3>常规课</h3><p>从 C++ 零基础到算法与数据结构，承接现有有道课程和课后题。</p><b>4 个阶段 · 70+ 课时</b><i>进入课程 →</i></button>
          <button class="path-card gesp" data-route="gesp"><span class="path-index">02</span><div class="path-icon">1–8</div><h3>考级课</h3><p>按 CCF GESP C++ 官方能力阶梯，从 1 级系统走到 8 级。</p><b>8 个等级 · 每课 2 小时</b><i>查看等级 →</i></button>
  ...
  function regularPage() {
    const staged = (window.REGULAR_L3_PROBLEMS || []).length;
    return shell(`<section class="page-hero compact"><span class="kicker">常规课学习链接</span><h1>基础能力不是前菜，<br>它是后面所有算法的地基。</h1><p>保留有道式课程层级，把讲义、关卡、课后题与进度收进同一条学习路径。</p></section>
      <section class="section"><div class="course-grid">${A.regularCourses.map(c=>`<article class="course-card"><div class="course-cover"><span>${c.level}</span><b>${c.status==='ready'?'可学习':'规划中'}</b></div><div class="course-body"><div class="tags">${c.tags.map(t=>`<span>${esc(t)}</span>`).join('')}</div><h3>${c.title}</h3><p>${c.subtitle}</p><div class="course-meta"><span>${c.lessons} 课</span><span>${c.duration}</span></div><button ${c.id==='regular-l3'?'data-route="regular-l3"':''}>${c.id==='regular-l3'?`查看 ${staged || 31} 道课后题`:'查看课程大纲'}</button></div></article>`).join('')}</div></section>`, 'regular');
  }

  function regularL3Page() {
    const problems = window.REGULAR_L3_PROBLEMS || [];
    return shell(`<section class="page-hero compact"><span class="kicker">常规课 · L3</span><h1>算法与数据结构入门</h1><p>已整理 ${problems.length || 31} 道有道 L3 真实课后题。题面、样例和已验证代码会逐题进入正式关卡。</p></section><section class="section"><div class="problem-list">${problems.length ? problems.map((p,i)=>`<article><span class="problem-no">${String(i+1).padStart(2,'0')}</span><div><strong>${esc(p.title)}</strong><p>${esc(p.statement).slice(0,120)}${p.statement.length>120?'…':''}</p><div class="tags"><span>难度 ${p.difficulty||2}</span><span>${esc(p.source||'有道 L3')}</span></div></div><button data-problem-preview="${esc(p.id)}">查看题面</button></article>`).join('') : `<div class="empty-state"><strong>31 道真实题已在暂存区</strong><p>正在做最后的数据格式转换；不影响 CSP-J 主线使用。</p></div>`}</div></section>`, 'regular');
  }

  function gespPage() {
    return shell(`<section class="page-hero compact gesp-hero"><span class="kicker">CCF GESP · C++</span><h1>1–8 级，不跳级地长出能力。</h1><p>低等级验证基础编程，高等级衔接 CSP-J/S；每课仍按 2 小时闭环编排。</p></section><section class="section"><div class="gesp-road">${A.gesp.map((g,i)=>`<article><div class="grade-orb">${i+1}</div><div class="grade-line"></div><div class="grade-content"><span>${g.grade} · ${g.lessons} 课 / ${g.duration}</span><h3>${g.title}</h3><p>${g.scope}</p><b>过级结果：${g.outcome}</b><button>查看 ${g.grade} 课程地图</button></div></article>`).join('')}</div><div class="official-note"><strong>课程边界以 CCF GESP 官方大纲为准</strong><p>网站把官方知识点重新排成可教、可练、可复习的 2 小时课，不私自改考试范围。</p><a href="${A.sources.gespSyllabus}" target="_blank" rel="noreferrer">查看官方大纲 →</a></div></section>`, 'gesp');
  }

  function levelState(index) {
    const s = store.get(); if (s.teacherMode) return 'open';
    if (s.completedLessons.includes(lessons[index].id)) return 'done';
    if (index === 0 || s.completedLessons.includes(lessons[index-1].id)) return 'current';
    return 'locked';
  }

  function campPage() {
    const s = store.get();
    return shell(`<section class="camp-hero"><div><span class="kicker light">CSP-J 集训课</span><h1>第一轮拿资格，<br>第二轮写真代码。</h1><p>知识关、专项关、真题 Boss 三层递进；过关分数线是 80 分。</p><div class="camp-meta"><span>每课 120 分钟</span><span>进度自动保存</span><span>${s.teacherMode?'答案与授课点已解锁':'学生模式不提前展示答案'}</span></div></div><div class="camp-score"><strong>${Math.round(s.completedLessons.length/lessons.length*100)}%</strong><span>初赛主线进度</span><button data-route="plan">排我的冲刺计划</button></div></section>
      <section class="section track-section"><div class="track-head"><div><span class="track-number">第一轮</span><h2>初赛知识闯关</h2></div><p>8 关 × 2 小时 + 历年真题 Boss</p></div><div class="level-map">${lessons.map((l,i)=>{const state=levelState(i);const score=s.bestScores[l.id]||0;return `<article class="level-node ${state}" ${state!=='locked'?`data-route="lesson/${l.id}"`:''}><div class="node-pin"><span>${state==='done'?'✓':state==='locked'?'锁':l.id}</span></div><div class="node-card"><div><span>第 ${i+1} 关 · 120 分钟</span>${score?`<b>最高 ${score} 分</b>`:''}</div><h3>${l.title}</h3><p>${l.goal}</p><i>${state==='done'?'已通关 · 可复习':state==='current'||state==='open'?'开始学习 →':'先完成上一关'}</i></div></article>`}).join('')}<article class="level-node boss" data-route="exams"><div class="node-pin"><span>★</span></div><div class="node-card"><div><span>最终 Boss</span><b>2015–2025</b></div><h3>历年第一轮真题</h3><p>4 届 NOIP 普及组历史卷 + 7 届 CSP-J 正式卷，限时完成并按错因复盘。</p><i>进入真题库 →</i></div></article></div></section>
  ...
  function lessonPage(id) {
    ...
    const s = store.get(); const locked = !s.teacherMode && levelState(lessons.indexOf(l)) === 'locked';
  ...
  function quizSummary(lesson, result) {
    return `<div class="quiz-summary ${result.score>=80?'pass':'retry'}"><strong>${result.score>=80?'过关！下一关已解锁':'还差一点，先补错因'}</strong><span>${result.score} 分</span><p>${result.score>=80?'现在不要立刻重刷，把错题加入间隔复习，明天再检验一次。':`错了 ${result.wrongIds.length} 题。回到对应讲义和记忆卡，再做一次同类变式。`}</p></div>`;
  }

  function examsPage() {
    return shell(`<section class="page-hero compact"><span class="kicker">历年真题 Boss</span><h1>11 个年度，不伪装成“10 届 CSP-J”。</h1><p>CSP-J 始于 2019 年；为了覆盖近十年训练，2015–2018 明确标为 NOIP 普及组历史衔接卷，2019–2025 才是 CSP-J。</p></section><section class="section exam-section"><div class="exam-tabs"><button class="active">第一轮 · 笔试</button><button data-route="second-exams">第二轮 · 上机</button></div><div class="exam-grid">${A.firstRound.map(x=>`<article><div><span>${x.year}</span><b>${x.family}</b></div><h3>${x.name}</h3><p>${x.note}</p><div class="exam-actions">${x.localPaper?`<a href="${x.localPaper}" target="_blank">打开本地试卷</a>`:'<button disabled>题面待核验</button>'}${x.localAnswer?`<a class="answer-link teacher-only ${store.get().teacherMode?'show':''}" href="${x.localAnswer}" target="_blank">参考答案</a>`:''}</div></article>`).join('')}</div><div class="integrity-note"><strong>内容诚信线</strong><p>本地有卷才显示“打开试卷”；缺卷年份明确写“待核验”，不会用样例题冒充真题。答案入口只在教师模式出现。</p></div></section>`, 'exams');
  }
  ...
  function notFound(){return shell('<section class="locked-page"><strong>这个页面还没有开课</strong><p>先回首页选择常规课、考级课或集训课。</p><button class="primary" data-route="home">返回首页</button></section>','');}
  ...
  function bind() {
    ...
    document.querySelectorAll('input[type=radio]').forEach(el=>el.onchange=()=>{const lessonId=pathParts()[1];ui.quizAnswers[lessonId]=Object.assign({},ui.quizAnswers[lessonId],{[el.name]:Number(el.value)})});
    const quiz=document.getElementById('lesson-quiz');if(quiz)quiz.onsubmit=e=>{e.preventDefault();const id=pathParts()[1],l=lessonById(id),answers=ui.quizAnswers[id]||{};if(Object.keys(answers).length<l.quiz.length){toast('还有题目没作答','error');return}const wrong=l.quiz.filter(q=>answers[q.id]!==q.answer).map(q=>q.id);const score=Math.round((l.quiz.length-wrong.length)/l.quiz.length*100);ui.quizResults[id]={score,wrongIds:wrong};store.recordQuiz(id,score,wrong);toast(score>=80?'过关，下一关已解锁':'已生成错因修复队列',score>=80?'success':'error');render()};
